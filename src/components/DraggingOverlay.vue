<template>
  <Transition aria-busy="true" name="drag-overlay">
    <div
      v-if="isVisible"
      aria-busy="true"
      class="fixed pointer-events-none inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm border-2 border-primary border-dashed m-4 rounded-xl">
      <div class="flex flex-col items-center justify-center space-y-6 min-w-120 max-w-screen w-1/2 h-1/2">
        <div class="relative">
          <div class="relative rounded-full bg-primary/10 p-6">
            <Icon :icon="icon" class="h-16 w-16 text-primary" />
          </div>
        </div>

        <div class="text-center space-y-2">
          <h3 class="text-xl font-semibold text-foreground">{{ title }}</h3>
          <p class="text-muted-foreground text-base">{{ subtitle }}</p>
        </div>
